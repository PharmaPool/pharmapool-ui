...
function Info() {
  const services = [
    ...
    {
      type: "Pharmacy Profile",
      details: [
        "Register all your pharmacies",
        "Create inventories for all products",
  ...
  return (
    <div className="info">
      ...
      <div className="home_services">
        ...
        <div className="home_serv">
          <div className="service_details">
            <h1>{services[4].type}</h1>
            <ul>
              {services[4].details.map((detail, i) => (
                <p key={i}>{detail}</p>
              ))}
            </ul>
          </div>
          <div className="service_img">
            <img src={images.inventory} alt="demand_image" />
          </div>
        </div>
      </div>
    </div>
